...
class RiscForthCompiler:
    def __init__(self):
        ...
        self.builtins = {
            ...
            '=': self._gen_eq, '<': self._gen_lt, '>': self._gen_gt, '<>': self._gen_ne, '<=': self._gen_le,
            '>=': self._gen_ge,
            'dup': self._gen_dup, 'drop': self._gen_drop, 'swap': self._gen_swap, 'over': self._gen_over,
            'rot': self._gen_rot,
    ...
    def _gen_op(self, op: Opcode):
        self.code.extend([
            ...
            Instruction(op, rd=Reg.T1.value, rs=Reg.T1.value, rt=Reg.T2.value),
            Instruction(Opcode.PUSH, rs=Reg.T1.value)
        ])

    def _gen_add(self):
        self._gen_op(Opcode.ADD)

    def _gen_sub(self):
        self._gen_op(Opcode.SUB)

    def _gen_mul(self):
        self._gen_op(Opcode.MUL)

    def _gen_div(self):
        self._gen_op(Opcode.DIV)

    def _gen_mod(self):
        self._gen_op(Opcode.MOD)

    def _gen_and(self):
        self._gen_op(Opcode.AND)

    def _gen_or(self):
        self._gen_op(Opcode.OR)

    def _gen_xor(self):
        self._gen_op(Opcode.XOR)

    def _gen_shl(self):
        self._gen_op(Opcode.SHL)

    def _gen_shr(self):
        self._gen_op(Opcode.SHR)

    def _gen_eq(self):
        self._gen_op(Opcode.CMP)

    def _gen_ne(self):
        self._gen_eq(); self._gen_not()

    def _gen_lt(self):
        self._gen_op(Opcode.SUB); self._emit_literal(31); self._gen_op(Opcode.SHR)

    def _gen_gt(self):
        self._gen_swap(); self._gen_lt()

    def _gen_le(self):
        self._gen_gt(); self._gen_not()

    def _gen_ge(self):
        self._gen_lt(); self._gen_not()

    def _gen_dup(self):
        self.code.extend([Instruction(Opcode.POP, rt=Reg.T1.value), Instruction(Opcode.PUSH, rs=Reg.T1.value),
                          Instruction(Opcode.PUSH, rs=Reg.T1.value)])
    ...
    def _gen_swap(self):
        self.code.extend([Instruction(Opcode.POP, rt=Reg.T1.value), Instruction(Opcode.POP, rt=Reg.T2.value),
                          Instruction(Opcode.PUSH, rs=Reg.T1.value), Instruction(Opcode.PUSH, rs=Reg.T2.value)])

    def _gen_over(self):
        self.code.extend([Instruction(Opcode.POP, rt=Reg.T1.value), Instruction(Opcode.POP, rt=Reg.T2.value),
                          Instruction(Opcode.PUSH, rs=Reg.T2.value), Instruction(Opcode.PUSH, rs=Reg.T1.value),
                          Instruction(Opcode.PUSH, rs=Reg.T2.value)])

    def _gen_rot(self):
        self.code.extend([Instruction(Opcode.POP, rt=Reg.T1.value), Instruction(Opcode.POP, rt=Reg.T2.value),
                          Instruction(Opcode.POP, rt=Reg.A0.value), Instruction(Opcode.PUSH, rs=Reg.T2.value),
                          Instruction(Opcode.PUSH, rs=Reg.T1.value), Instruction(Opcode.PUSH, rs=Reg.A0.value)])

    def _gen_not(self):
        self._emit_literal(0); self._gen_eq()

    def _gen_zero_eq(self):
        self._gen_not()

    def _gen_fetch(self):
        self.code.extend([Instruction(Opcode.POP, rt=Reg.T1.value),
                          Instruction(Opcode.LOAD, rt=Reg.T1.value, rs=Reg.T1.value, imm=0),
                          Instruction(Opcode.PUSH, rs=Reg.T1.value)])

    def _gen_store(self):
        self.code.extend([Instruction(Opcode.POP, rt=Reg.T1.value), Instruction(Opcode.POP, rt=Reg.T2.value),
                          Instruction(Opcode.STORE, rs=Reg.T2.value, rt=Reg.T1.value, imm=0)])

    def _gen_emit(self):
        self.code.extend(
            [Instruction(Opcode.POP, rt=Reg.T1.value), Instruction(Opcode.OUT, rs=Reg.T1.value, imm=IO_OUTPUT_PORT)])

    def _gen_key(self):
        self.code.extend(
            [Instruction(Opcode.IN, rt=Reg.T1.value, imm=IO_INPUT_PORT), Instruction(Opcode.PUSH, rs=Reg.T1.value)])
    ...
    def _gen_string_literal(self, text: str):
        ...
        for char in text: self.data_segment.append(ord(char))
        self.data_addr += len(text) + 1
        self._emit_literal(string_addr)

    def _compile_tokens(self, tokens: List[Token]):
        """Вспомогательный метод для компиляции последовательности токенов."""
        i = 0
        while i < len(tokens):
            token = tokens[i]
            if token.type == 'WORD':
                ...
                self.if_stack.append(len(self.code) - 1)
            elif token.type == 'ELSE':
                if_jump_addr = self.if_stack.pop()
                else_jump_instr = Instruction(Opcode.JMP, addr=0)
                self.code.append(else_jump_instr)
                self.code[if_jump_addr].imm = len(self.code)
                self.if_stack.append(len(self.code) - 1)
            elif token.type == 'THEN':
                jump_addr = self.if_stack.pop()
                if self.code[jump_addr].opcode == Opcode.JMP:
                    self.code[jump_addr].addr = len(self.code)
                else:
                    self.code[jump_addr].imm = len(self.code)
            i += 1

    def compile(self, tokens: List[Token]) -> List[Instruction]:
        logging.info("Начинаем компиляцию Forth кода")

        # Добавляем JMP на основной код (адрес пока неизвестен)
        jump_to_main = Instruction(Opcode.JMP, addr=0)
        self.code.append(jump_to_main)
        ...
        while i < len(tokens):
            if tokens[i].type == 'COLON':
                ...
                while i < len(tokens) and tokens[i].type != 'SEMICOLON':
                    proc_body.append(tokens[i])
                    i += 1

                self._compile_tokens(proc_body)
                self.code.append(Instruction(Opcode.RET))
            i += 1

        # Устанавливаем адрес для JMP на основной код
        main_code_start_addr = len(self.code)
        jump_to_main.addr = main_code_start_addr

        #  Компиляция основного кода (все, что не является процедурой)
        main_body_tokens = []
        in_proc_def = False
        for token in tokens:
            if token.type == 'COLON': in_proc_def = True
            if not in_proc_def: main_body_tokens.append(token)
            if token.type == 'SEMICOLON': in_proc_def = False

        self._compile_tokens(main_body_tokens)
        self.code.append(Instruction(Opcode.HALT))
    ...
    def save_binary(self, filename: str):
        with open(filename, 'wb') as f:
            for instruction in self.code: f.write(instruction.to_binary())
            for val in self.data_segment: f.write(struct.pack('>I', val))

        with open(filename + '.hex', 'w') as f:
            f.write(f"; Instructions: {len(self.code)}\n")
            for i, instr in enumerate(self.code): f.write(instr.to_hex(i) + '\n')  # Используем индекс, а не адрес
            f.write("\n; Data Segment\n")
            data_start_addr = len(self.code) * 4
            for i, val in enumerate(self.data_segment):
                f.write(f"0x{data_start_addr + i * 4:04X}: {val:08X} ({repr(chr(val)) if 32 <= val <= 126 else '.'})\n")
...
class RiscForthTranslator:
    ...
    def translate_file(self, source_path: str, output_path: str):
        with open(source_path, 'r', encoding='utf-8') as f: source = f.read()
